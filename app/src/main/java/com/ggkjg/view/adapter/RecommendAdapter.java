...
public class RecommendAdapter extends BaseQuickAdapter<RecommendDto, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, RecommendDto item) {
        ...
        String memberName = "";
        if (Integer.valueOf(item.getMemberLevel()) == MemberLevelType.level_1.getType()) {
            memberName = MemberLevelType.level_1.getValue();
//            helper.setGone(R.id.img_level, false)
            helper.setImageResource(R.id.img_level, R.mipmap.icon_member_level_1)
                    .setTextColor(R.id.tv_level_name, mContext.getResources().getColor(R.color.my_color_666666));
        } else if (Integer.valueOf(item.getMemberLevel())== MemberLevelType.level_2.getType()) {
            ...
            ;
        } else if (Integer.valueOf(item.getMemberLevel()) == MemberLevelType.level_3.getType()) {
            ...
            ;
        } else if (Integer.valueOf(item.getMemberLevel()) == MemberLevelType.level_4.getType()) {
