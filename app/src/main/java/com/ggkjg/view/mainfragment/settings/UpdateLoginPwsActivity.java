 ...
 */
public class UpdateLoginPwsActivity extends BaseActivity {
    ...
    public int getLayoutId() {
        return R.layout.ui_setting_update_login_pwd_layout;
    }
    ...
    public void initListener() {
        bindClickEvent(btnSure, () -> {
            ...
            if (TextUtils.isEmpty(oldPwdStr)) {
                ToastUtil.showToast("请输入原登录密码");
                return;
            }
            if (TextUtils.isEmpty(newPwdStr)) {
                ToastUtil.showToast("请输入新登录密码");
                return;
            }
            if (TextUtils.isEmpty(newPwdRepeatStr)) {
                ToastUtil.showToast("请输入新登录密码确定");
                return;
            }
            if (!newPwdStr.equals(newPwdRepeatStr)) {
                ToastUtil.showToast("新登录密码与确定新登录密码不同");
                return;
            }
    ...
    private void modifyPwd(HashMap<String, String> map) {
        showLoadDialog();
        DataManager.getInstance().modifyLoginPwd(new DefaultSingleObserver<HttpResult<Object>>() {
            @Override
