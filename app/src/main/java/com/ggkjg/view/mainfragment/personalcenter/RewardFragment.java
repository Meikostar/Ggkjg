...
import com.ggkjg.view.adapter.RewardAdapter;
...
public class RewardFragment extends BaseFragment {
    ...
    RewardActivity.RewardInfoListener rewardInfoListener;
    ...
    private void loadData(boolean isLoad) {
        ...
        if (!TextUtils.isEmpty(type)) {
            map.put("bonusType", type);
        }

        DataManager.getInstance().findMyBonus(new DefaultSingleObserver<BoundsDataDto>() {
            @Override
            public void onSuccess(BoundsDataDto boundsDataDto) {
                dissLoadDialog();
                if (boundsDataDto != null) {
                    if (boundsDataDto.getBonusDetail() != null) {
                        if (mCurrentPage == Constants.PAGE_NUM) {
                            mAdapter.setNewData(boundsDataDto.getBonusDetail().getRows());
                            if (boundsDataDto.getBonusDetail().getRows() == null || boundsDataDto.getBonusDetail().getRows().size() == 0) {
                                mAdapter.setEmptyView(new EmptyView(getActivity()));
                            }
                        } else {
                            mAdapter.addData(boundsDataDto.getBonusDetail().getRows());
                        }

                    }
                    //全部
                    if (rewardInfoListener != null && Constants.PAGE_NUM == mCurrentPage && TextUtils.isEmpty(type) && boundsDataDto.getBonusAccmount() != null) {
                        rewardInfoListener.baseInfoBack(boundsDataDto.getBonusAccmount());
                    }
                }
            }
    ...
    public void setRewardInfoListener(RewardActivity.RewardInfoListener rewardInfoListener) {
        this.rewardInfoListener = rewardInfoListener;
    }
}
