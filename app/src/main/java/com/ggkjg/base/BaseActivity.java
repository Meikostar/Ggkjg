...
import com.ggkjg.view.mainfragment.login.ForgetPwdActivity;
...
public abstract class BaseActivity extends AppCompatActivity implements BaseViewInterface {
    ...
    public boolean isTopActivity(int type) {
        boolean isTop = false;
        if (activityList != null && activityList.size() > 0) {
            if (type == 0 && activityList.get(activityList.size() - 1) instanceof RegisterActivity) {
                isTop = true;
            } else if (type == 1 && activityList.get(activityList.size() - 1) instanceof ForgetPwdActivity) {
                isTop = true;
            }
    ...
    public void checkPermissioin() {
        ActivityCompat.requestPermissions(BaseActivity.this, new String[]{WRITE_EXTERNAL_STORAGE,
                READ_EXTERNAL_STORAGE, ACCESS_FINE_LOCATION, CALL_PHONE, CAMERA}, REQUEST_CODE_ADDRESS);
    }
